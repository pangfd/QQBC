...
void variant_snapshot_reader::validate() const {
   QQBC_ASSERT(snapshot.is_object(), snapshot_validation_exception,
         "Variant snapshot is not an object");
   const fc::variant_object& o = snapshot.get_object();

   QQBC_ASSERT(o.contains("version"), snapshot_validation_exception,
         "Variant snapshot has no version");

   const auto& version = o["version"];
   QQBC_ASSERT(version.is_integer(), snapshot_validation_exception,
         "Variant snapshot version is not an integer");

   QQBC_ASSERT(version.as_uint64() == (uint64_t)current_snapshot_version, snapshot_validation_exception,
   ...
   QQBC_ASSERT(o.contains("sections"), snapshot_validation_exception,
         "Variant snapshot has no sections");

   const auto& sections = o["sections"];
   QQBC_ASSERT(sections.is_array(), snapshot_validation_exception, "Variant snapshot sections is not an array");

   const auto& section_array = sections.get_array();
   for( const auto& section: section_array ) {
      QQBC_ASSERT(section.is_object(), snapshot_validation_exception, "Variant snapshot section is not an object");

      const auto& so = section.get_object();
      QQBC_ASSERT(so.contains("name"), snapshot_validation_exception,
            "Variant snapshot section has no name");

      QQBC_ASSERT(so["name"].is_string(), snapshot_validation_exception,
                 "Variant snapshot section name is not a string");

      QQBC_ASSERT(so.contains("rows"), snapshot_validation_exception,
                 "Variant snapshot section has no rows");

      QQBC_ASSERT(so["rows"].is_array(), snapshot_validation_exception,
...
void variant_snapshot_reader::set_section( const string& section_name ) {
   ...
   QQBC_THROW(snapshot_exception, "Variant snapshot has no section named ${n}", ("n", section_name));
}
...
{
   QQBC_ASSERT(section_pos == std::streampos(-1), snapshot_exception, "Attempting to write a new section without closing the previous section");
...
void istream_snapshot_reader::validate() const {
   ...
   try {
      ...
      QQBC_ASSERT(actual_totem == expected_totem, snapshot_exception,
      ...
      QQBC_ASSERT(actual_version == expected_version, snapshot_exception,
...
void istream_snapshot_reader::set_section( const string& section_name ) {
   ...
   QQBC_THROW(snapshot_exception, "Binary snapshot has no section named ${n}", ("n", section_name));
}
