...
###################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL QQBCIO BEFORE THESE WILL PASS #
###################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        [[ ! -z $(echo "${output}" | grep "QQBCIO compiler checks require the 'which'") ]] || exit
    fi
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "QQBCIO_INSTALL_DIR: ${HOME}/eosio/${QQBCIO_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s QQBC2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: QQBC2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "QQBCIO_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "QQBCIO_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
