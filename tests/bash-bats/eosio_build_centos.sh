...
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting QQBCIO Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting QQBCIO Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep "make -j${CPU_CORES}") ]] || exit
    [[ ! -z $(echo "${output}" | grep "QQBCIO has been successfully built") ]] || exit
